...
OUTPUT_DIR=$1
TARGET_NAME=plugin_applovinMax
OUTPUT_SUFFIX=a
CONFIG=Release
...
build_plugin_structure() {
	...
	PLATFORM=$2
	ARCH=$3
	BPATH="$path/build/$CONFIG-$PLATFORM"
	cp "$BPATH/lib$TARGET_NAME.$OUTPUT_SUFFIX" "$PLUGIN_DEST/"
	cp "$path"/metadata.lua "$PLUGIN_DEST/"

	if ls "$path"/EmbeddedFrameworks/*.framework 1> /dev/null 2>&1 || ls "$BPATH/"*.framework 1> /dev/null 2>&1
	then
		echo "Copying frameworks frameworks for $PLATFORM:"

		for f in "$path"/EmbeddedFrameworks/*.framework "$BPATH"/*.framework
		do
			if [ ! -d "$f" ]; then
				continue
			fi
